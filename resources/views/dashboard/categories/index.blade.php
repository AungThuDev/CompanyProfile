...
<div class="max-w-6xl mx-auto space-y-4">
    ...
    <div class="flex justify-between items-center bg-white rounded-lg border border-gray-200 p-4">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Manage all categories in your platform</p>
        </div>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse($categories as $category)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 text-sm text-gray-500">{{ Str::limit($category->description, 60) ?? 'N/A' }}</td>
                        <td class="px-4 py-2 text-sm text-gray-500">{{ $category->created_at->format('M d, Y') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-4 text-center text-gray-500 text-sm">
                            No categories found. 
                            <a href="{{ route('dashboard.categories.create') }}" class="text-indigo-600 underline">Create one</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
