                ...
                <div class="flex items-center gap-4">
                    <div class="w-24 h-24 rounded-lg overflow-hidden ring-2 ring-indigo-100">
                        @if($service->icon && file_exists(storage_path('app/public/' . $service->icon)))
                            <img src="{{ asset('storage/' . $service->icon) }}" class="w-full h-full object-contain bg-gray-50 p-2">
                        @else
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Change Icon</label>
                        <input type="file" name="icon" accept="image/*" class="block w-full text-sm text-gray-600">
                        <p class="text-xs text-gray-400 mt-1">Optional. JPG, JPEG, PNG, WEBP, SVG. Max 2MB.</p>
                    </div>
                </div>
