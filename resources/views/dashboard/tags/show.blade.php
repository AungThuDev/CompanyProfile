...
<div class="max-w-4xl mx-auto my-8 space-y-4">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4">
        <h1 class="text-lg font-semibold text-gray-900">{{ $tag->name }}</h1>
        <p class="text-xs text-gray-500 mt-1">Detailed information about this tag</p>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-gray-900">{{ $tag->slug }}</p>
            </div>

            <div class="col-span-2">
                <label class="block text-xs font-medium text-gray-700">Description</label>
                <p class="mt-1 text-gray-900">{{ $tag->description ?? 'N/A' }}</p>
            </div>

            <div>
                <label class="block text-xs font-medium text-gray-700">Creator</label>
                <p class="mt-1 text-gray-900">{{ $tag->creator->name ?? 'N/A' }}</p>
            </div>

            <div>
                <label class="block text-xs font-medium text-gray-700">Updater</label>
                <p class="mt-1 text-gray-900">{{ $tag->updator->name ?? 'N/A' }}</p>
            </div>

            <div>
                <label class="block text-xs font-medium text-gray-700">Created At</label>
                <p class="mt-1 text-gray-900">{{ $tag->created_at->format('M d, Y') }}</p>
            </div>

            <div>
                <label class="block text-xs font-medium text-gray-700">Updated At</label>
                <p class="mt-1 text-gray-900">{{ $tag->updated_at->format('M d, Y') }}</p>
            </div>
