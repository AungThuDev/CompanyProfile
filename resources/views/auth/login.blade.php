...
<div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
   ...
   {{-- Error Messages --}}
    ...
    {{-- Session Error --}}
    @if (session('error'))
    <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded-md text-sm shadow-sm mb-4">
        {{ session('error') }}
    </div>
    @endif
    ...
    <form action="{{ route('auth.login') }}" method="POST" class="space-y-4">
        ...
        <div class="text-right text-sm">
            <a href="{{ route('auth.forgot-password') }}" class="text-indigo-600 hover:text-indigo-700 font-medium">
                Forgot Password?
            </a>
        </div>
